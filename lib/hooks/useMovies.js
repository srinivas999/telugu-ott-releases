...
function findBestTmdbMatch(results, title) {
  ...
  const exactMatch = results.find((movie) => {
    const candidate = normalizeTitle(movie.title || movie.original_title || movie.name || movie.original_name);
...
async function searchTmdbMovieByTitle(title, category, options = {}) {
  if (!title) {
    return null;
  }
  ...
  const originalTitle = String(options.originalTitle || title).trim();
  ...
  if (isGitHubDeploy) {
    ...
    response = await fetch(
      `https://api.themoviedb.org/3/search/${mediaType}?api_key=${encodeURIComponent(tmdbApiKey)}&query=${encodeURIComponent(title)}&language=te-IN&region=IN&page=1&include_adult=false`
    );
  } else {
    response = await fetch(
      `/api/tmdb/search?query=${encodeURIComponent(title)}&originalTitle=${encodeURIComponent(originalTitle)}&year=${encodeURIComponent(year)}&mediaType=${encodeURIComponent(mediaType)}`
    );
  }
  ...
  return payload.bestMatch || findBestTmdbMatch(payload.results, title);
}
...
async function enrichMoviesWithTmdb(movies) {
  const enrichedMovies = await Promise.all(
    movies.map(async (movie) => {
      ...
      try {
        const tmdbMovie = await searchTmdbMovieByTitle(movie.movie_name, movie.category, {
          originalTitle: movie.original_title || movie.movie_name,
          year: String(movie.release_date || movie.digital_release_date || '').slice(0, 4),
        });
        if (!tmdbMovie) {
          return movie;
        }
        ...
        return {
          ...movie,
          title: movie.title || tmdbMovie.title || tmdbMovie.name || movie.movie_name,

...
/**
 * Hook to fetch trending movies (highest rated) with 304 caching
 * @param {number} limit - Number of movies to fetch
 * @returns {object} - { movies, loading, error }
 */
export function useTrendingMovies(limit = 10) {
  ...
  useEffect(() => {
    async function loadTrending() {
      ...
      try {
        const cacheKey = `tmdb_trending_${limit}`;
        const cached = typeof window !== 'undefined' ? window.localStorage.getItem(cacheKey) : null;
        const cachedData = cached ? JSON.parse(cached) : null;
        const etag = cachedData?.etag || null;

        const res = await fetch(`/api/tmdb/trending?limit=${encodeURIComponent(limit)}`, {
        ...
        if (!res.ok) {
          ...
          throw new Error(`TMDB trending request failed (${res.status}): ${body}`);
        }
        ...
        if (typeof window !== 'undefined' && newETag) {

...
function getMovieName(movie) {
  return movie?.movie_name || movie?.title || movie?.name || 'Untitled';
}

function getMovieUrl(movie, baseUrl = SITE_URL) {
  if (!movie?.id) return undefined;
  return `${baseUrl}/movie/${generateUniqueSlug(getMovieName(movie), movie.id)}`;
}
...
function getMovieGenres(movie) {
  ...
  if (Array.isArray(movie?.genres)) {
    return movie.genres.map((genre) => genre?.name || genre).filter(Boolean);
  }

  if (typeof movie?.genres === 'string') {
    return movie.genres.split(',').map((genre) => genre.trim()).filter(Boolean);
  }
...
export function generateMovieListItem(movie, position, baseUrl = SITE_URL) {
  if (!movie) return null;

  const rating = getPreferredMovieRating(movie);
  const explicitUrl = movie?.url ? toAbsoluteUrl(movie.url) : undefined;
  const movieSchema = {
    '@type': 'Movie',
    name: getMovieName(movie),
    ...(movie.digital_release_date || movie.release_date
      ? { datePublished: movie.digital_release_date || movie.release_date }
      : {}),
    ...(movie.streaming_partner
      ? { description: `Streaming on ${movie.streaming_partner}` }
      : movie.overview || movie.description
        ? { description: movie.overview || movie.description }
        : {}),
    ...(movie.language || movie.movie_language
      ? { inLanguage: movie.language || movie.movie_language }
      : {}),
    ...(toPosterImage(movie) ? { image: toPosterImage(movie) } : {}),
    ...(explicitUrl || getMovieUrl(movie, baseUrl) ? { url: explicitUrl || getMovieUrl(movie, baseUrl) } : {}),
    ...(getMovieGenres(movie).length > 0 ? { genre: getMovieGenres(movie) } : {}),
    ...(rating
      ? {
          aggregateRating: {
...
export function generateMovieSchema(movie, baseUrl = 'https://svteluguott.in') {
  ...
  const schema = {
    ...
    name: title,
    description: movie.overview || movie.description || '',
    image: toPosterImage(movie),
    ...(movie.id && {
      url: getMovieUrl(movie, baseUrl),
    }),
    ...(movie.backdrop_path && {
...
export function generateItemListSchema(config) {
  ...
  return {
    ...
    itemListElement: items
      .map((item, index) => generateMovieListItem(item, index + 1))
      .filter(Boolean),
  };
}
